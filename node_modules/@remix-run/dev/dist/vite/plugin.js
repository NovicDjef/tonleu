...
const resolveBuildAssetPaths = (ctx, viteManifest, entryFilePath, prependedAssetFilePaths = []) => {
...
let getServerBuildDirectory = ctx => path__namespace.join(ctx.remixConfig.buildDirectory, "server", ...(typeof ctx.serverBundleId === "string" ? [ctx.serverBundleId] : []));
...
const remixVitePlugin = (remixUserConfig = {}) => {
  ...
  let updateRemixPluginContext = async () => {
    var _viteUserConfig$serve, _viteUserConfig$build, _getServerBundleBuild;
    ...
    let ssrBuildCtx = viteConfigEnv.isSsrBuild && viteCommand === "build" ? {
      ...
      serverBundleId: (_getServerBundleBuild = getServerBundleBuildConfig(viteUserConfig)) === null || _getServerBundleBuild === void 0 ? void 0 : _getServerBundleBuild.serverBundleId
    } : {
      isSsrBuild: false
    };
    ctx = {
  ...
  let getServerEntry = async () => {
    ...
    ${Object.keys(ctx.remixConfig.routes).map((key, index) => {
  ...
  let createBrowserManifestForBuild = async () => {
    let viteManifest = await loadViteManifest(getClientBuildDirectory(ctx.remixConfig));
    let entry = resolveBuildAssetPaths(ctx, viteManifest, ctx.entryClientFilePath);
    let routes = {};
    ...
    for (let [key, route] of Object.entries(ctx.remixConfig.routes)) {
      ...
      routes[key] = {
        ...
        ...resolveBuildAssetPaths(ctx, viteManifest, routeFilePath,
    ...
    await writeFileSafe(path__namespace.join(getClientBuildDirectory(ctx.remixConfig), manifestPath), `window.__remixManifest=${JSON.stringify(manifest)};`);
    return manifest;
  };
  let getBrowserManifestForDev = async () => {
    let routes = {};
    let routeManifestExports = await getRouteManifestModuleExports(viteChildCompiler, ctx);
    for (let [key, route] of Object.entries(ctx.remixConfig.routes)) {
      let sourceExports = routeManifestExports[key];
  ...
  return [{
    ...
    config: async (_viteUserConfig, _viteConfigEnv) => {
      ...
      return {
        ...
        ...(viteCommand === "build" ? {
          build: {
            cssMinify: ((_viteUserConfig$build2 = viteUserConfig.build) === null || _viteUserConfig$build2 === void 0 ? void 0 : _viteUserConfig$build2.cssMinify) ?? true,
    ...
    writeBundle: {
      ...
      async handler() {
        ...
        let clientAssetPaths = new Set(Object.values(clientViteManifest).flatMap(chunk => [...(chunk.css ?? []), ...(chunk.assets ?? [])]));

        // Handle `.css?url` files that only exist in SSR module graph
        let ssrCssUrlFilePaths = Object.values(ssrViteManifest).filter(chunk => chunk.file.endsWith(".css")).map(chunk => chunk.file);

        // Handle generic assets that only exist in SSR module graph
        let ssrChunkAssetPaths = Object.values(ssrViteManifest).flatMap(chunk => chunk.assets ?? []);
        let ssrAssetPaths = new Set([...ssrCssUrlFilePaths, ...ssrChunkAssetPaths]);
    ...
    async load(id) {
      switch (id) {
        ...
        case vmod.resolve(serverManifestId):
          {
            let browserManifest = ctx.isSsrBuild ? await ctx.getBrowserManifest() : await getBrowserManifestForDev();
            return `export default ${jsesc__default["default"](browserManifest, {
              es6: true
            })};`;
          }
        case vmod.resolve(browserManifestId):
          {
            ...
            let browserManifestString = jsesc__default["default"](browserManifest, {
              es6: true
            });
    ...
    async resolveId(id, importer, options) {
      ...
      throw Error([pc__default["default"].red(`Server-only module referenced by client`), "", `    '${id}' imported by '${importerShort}'`, "", "  See https://remix.run/docs/en/main/future/vite#splitting-up-client-and-server-code", ""].join("\n"));
    }
  }, {
    ...
    async transform(code, id, options) {
      ...
      return {
        code: removeExports.removeExports(code, SERVER_ONLY_ROUTE_EXPORTS),
        map: null
      };
    }
  }, {
